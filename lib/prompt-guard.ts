 ...
 *
 * 背景：2026-08 排查 Network Error / 上游 #sym:500 时曾误判为体积穿透，
 * 2026-08-27 生产环境实测纠偏：
 *   - 2.6MB 请求体 / 43 万字符多轮历史：200 成功（100s）
 *   - 单条 3MB 中文消息：200 成功
 *   - 长时生成 69s+：200 成功
 *   体积并非 #sym:500 的诱因；真凶是上游中继渠道故障（模型名不匹配 404、
 *   10 次/分钟限速 429、凭证池/无头浏览器随机故障被符号化为 #sym:500）。
 * 防线保留但定位为「最后兜底」，默认阈值大幅放宽，优先保全用户体验：
 *   - 默认总闸 100 万字符（实测 2.6MB 多轮历史可通，留出充足余量）
 *   - 默认单条软限 8 万字符（普通角色扮演消息完全不受影响）
 *   - 字节硬帽 3.5MB（逼近网关 4MB 容差前才介入）
 *
 * 三层防线：
 ...
 *  2026-08-27 实测纠偏：2.6MB/43 万字符多轮历史生产环境稳定通过，
 *  体积不是 #sym:500 的诱因，绝对上限回放宽至 300 万字符，
 *  让用户可以按需恢复到宽松预算。 */
...
/** 用户未配置时的默认值（2026-08-27 纠偏：此前 30 万/1.2 万过严，导致
 *  历史消息合并后被大量折叠、角色扮演体验受损。按生产实测放宽容许。） */
const DEFAULT_TOTAL_CHARS = 1000000;
const DEFAULT_SOFT_CHARS = 80000;

/** 巨型消息截断后保留的尾部长度 */
const TAIL_KEEP_CHARS = 400;
/**
 * 发送前字节硬帽（3.5MB）。
 * 2026-08-27 实测纠偏：2.6MB 请求体稳定通过网关，此前 2MB 保险丝过紧
 * 且药不对症（#sym:500 与体积无关）。硬帽上抬至 3.5MB，仅作为逼近
 * 网关 4MB 容差前的最后物理兜底，正常聊天永远不会触发。
 */
export const TOTAL_BYTES_ABS_CAP = 3.5 * 1024 * 1024;
